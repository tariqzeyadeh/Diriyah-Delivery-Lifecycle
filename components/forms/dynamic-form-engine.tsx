  ...
  return (
    <div className="grid gap-4 md:grid-cols-2">
      {schema.map((field) => {
        ...
        const span =
          field.type === 'repeatingTable' || field.type === 'evidence' || field.type === 'textarea'
            ? 'md:col-span-2'
            : ''
          ...
          <div key={field.name} className={`space-y-1.5 ${span}`}>
            ...
            {field.type === 'boolean' && (
              <label className="flex items-center gap-2 text-sm text-text">
                <input
                  type="checkbox"
                  className="size-4 accent-[var(--color-diriyah-primary)]"
                  disabled={locked}
            ...
            {field.type === 'date' && (
              <input
                ...
                onChange={(e) =>
                  set(field.name, e.target.value ? `${e.target.value}T00:00:00.000Z` : '')
                }
              />
            )}
